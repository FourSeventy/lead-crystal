...
public class SkillManager 
{
    ...
    public RenderData dumpRenderData()
    {
        ...
        renderData.data.add(new HashSet(this.knownSkills.keySet()));
        renderData.data.add(this.skillPoints);
    ...
    {
         Set<SkillID> oldSkills = (Set)oldData.data.get(0);
         Set<SkillID> newSkills = (Set)newData.data.get(0);
         ArrayList<SkillID> addList = new ArrayList();
         ArrayList<SkillID> removeList = new ArrayList();
         
         //generate remove list
         Iterator iter = oldSkills.iterator();       
         while(iter.hasNext())
         {
             SkillID name = (SkillID)iter.next();
             if(!newSkills.contains(name))
                 removeList.add(name);
         }
         
         //generate add list
          iter = newSkills.iterator();       
         while(iter.hasNext())
         {
             SkillID name = (SkillID)iter.next();
             if(!oldSkills.contains(name))
                 addList.add(name);
         }
         
         SceneObjectRenderDataChanges changes = new SceneObjectRenderDataChanges();
         
         ArrayList changesArray = new ArrayList();
         int fields = 0;
         
         if(!addList.isEmpty())
         {
             changesArray.add(addList.toArray()); 
             fields += 1;
         }
         
         if(!removeList.isEmpty())
         {
             changesArray.add(removeList.toArray()); 
             fields += 2;
         }
         
         if(!oldData.data.get(1).equals(newData.data.get(1)))
         {
             changesArray.add(newData.data.get(1));
             fields +=4;
         }
         
         changes.fields = fields;
         changes.data = changesArray.toArray();
    ...
    public void reconcileRenderDataChanges(long lastTime,long futureTime,SceneObjectRenderDataChanges renderDataChanges)
    {
        ...
        {
            addList.addAll(Arrays.asList((Object[])changeData.get(0)));
        ...
        {
            removeList.addAll(Arrays.asList((Object[])changeData.get(1)));
        }
        
        if(changeData.get(2) != null)
        {
            this.skillPoints = (int)changeData.get(2);
        }
        
        for(int i = 0; i < addList.size(); i++)
        {
            this.learnSkill((SkillID)addList.get(i));
        }
        
        for(int i = 0; i < removeList.size(); i++)
        {
            this.forgetSkill((SkillID)removeList.get(i));
        }
